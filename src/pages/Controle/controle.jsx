...
const Controle = () => {
  ...
  const getCategoryName = async (cateId) => {
    try {
      const response = await app.get(`/categoria/${cateId}`);
      return response.data.nome_categoria;
    } catch (error) {
      console.error(`Erro ao buscar nome da categoria para cateId ${cateId}`, error);
  ...
  const getEstoque = async () => {
    try {
      const response = await app.get('/cadItem/');
      const dataTable = response.data;
      
      const updatedData = await Promise.all(dataTable.map(async (item) => {
        const nomeCategoria = await getCategoryName(item.fk_categoriaId);
        return {
          ...item,
                  ...
                  {filteredData
                    ...
                    .map((item, index) => (
                      <S.TrBody key={index}>
                        ...
                        <S.StyledTableCell>{item.qtdMin}</S.StyledTableCell>
