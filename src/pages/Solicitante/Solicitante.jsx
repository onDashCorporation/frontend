import React, { useEffect, useState } from 'react'; // Importe React e hooks necessários
...
const solicitante = () => {
  ...
  useEffect(() => {
    // Requisição GET para carregar os pedidos ao montar o componente
    fetch('/controle')
  ...
  const handleSearch = (event) => {
    ...
    const newFilteredData = pedidos.filter((item) => {
      ...
      <S.Main>
        <S.Container>
          <S.Section>
            ...
            <S.Header>
              <S.Option>
                <S.Op select={opset ? 'true' : undefined} onClick={() => { setOpset(true) }}>Status</S.Op>
                <S.Op select={!opset ? 'false' : undefined} onClick={() => { setOpset(false) }}>Historico</S.Op>
              </S.Option>
              <S.InsertContainer>
                <S.SearchContainer>
                  <Search
